...
- Infrastructure（基础设施层）
  - 适配器实现：仓库（内存/Postgres）、资产写入（FS）、算法客户端、支付客户端
  - 面向接口的具体实现，可替换
- Config/Env（配置）
...
- 在 server.New 中根据配置初始化依赖：
  ...
  - 算法服务使用 algoAdapter（封装 IDPhoto、AddBackgroundBase64）
- 配置来源
...
- 任务创建（POST /api/tasks）
  ...
  - 用例调用 Algo.IDPhoto 获取 rgba base64 → For colors 调用 AddBackgroundBase64 → 写入资产（FSWriter）→ 写入 TaskRepo（状态与 URL）
- 任务查询/下载信息
...
## 算法与支付集成
- 算法服务（参考）
  - /idphoto：multipart input_image + height/width/dpi/hd/face_alignment
    - 返回 image_base64_standard / image_base64_hd 与 status
  - /add_background：multipart input_image_base64 + color + dpi
    - 返回 image_base64 与 status
  - Base64 解码兼容 data URL 前缀与补位处理
- 微信支付（mock）
...
## 测试与运行
- 前置
  - 算法服务需在 8080 运行（PERMIT_ALGO_URL 可配置）
...
## Docker 一键启动
- 通过 docker compose 启动后端 + PostgreSQL + 算法服务
...
- 默认端口
  ...
  - PostgreSQL：5432
- 关键环境变量
  - PERMIT_ALGO_URL=http://algo:8080
...
- 算法客户端：[client.go](file:///d:/dev/one-permit/permit-backend/internal/algo/client.go)
