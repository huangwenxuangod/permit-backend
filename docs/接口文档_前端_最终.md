...
- 说明：
  ...
  - `itemId` 传回执类目也可以，后端会自动转换为证件照规格生成 v5。
...
```json
{
  "taskId":"...",
  "urls":{"white":"...","blue":"..."},
  "layoutUrls":{"6inch":"...","receipt_white":"..."},
  "receiptUrls":{"white":"...","blue":"..."},
  "expiresIn":600
}
```
...
- `POST /api/pay/wechat/notify`
- 由微信支付回调，请勿前端调用
  - 订单状态变为 `paid` 后，后端会自动触发回执制作，并可通过下载接口获取 `receiptUrls`
